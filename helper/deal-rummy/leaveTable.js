...
const roundStartAction = require('./roundStart'); //import the spesific function
...
module.exports.manageOnUserLeave = async (tb, client) => {
  try {
    ...
    logger.info('<== manage On UserLeave playerInGame : ==> ', playerInGame, playerInGame.length);
  ...
  try {
    ...
    for (let i = 0; i < playerInfos.length; i++) {
      ...
      if (typeof playerInfos[i].seatIndex !== 'undefined') {
        ...
        logger.info("Deal remove robot tbInfo1", tbInfo1)
